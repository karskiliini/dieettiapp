...
function AppContent() {
  ...
  // Snapshot views during swipe so they survive state changes
  ...
  const goBackAnimated = useCallback(() => {
    ...
    setHistory((h) => {
      ...
      if (p) { setView(p); return h.slice(0, -1); }
  ...
  // Capture snapshots when swipe starts
  const isSwiping = swipe.isDragging || swipe.isCompleting;

  useEffect(() => {
    if (swipe.isDragging && !swipe.isCompleting) {
      // Starting a new drag — snapshot current views
      swipeFrontRef.current = view;
      swipeBackRef.current = prevView;
    }
    if (!isSwiping) {
      // Swipe ended — clear snapshots
      swipeFrontRef.current = null;
      swipeBackRef.current = null;
    }
  }, [swipe.isDragging, swipe.isCompleting, isSwiping]);
  // Intentionally NOT including view/prevView — we only want the initial snapshot
  ...
  const mealPlan = baseMealPlan.map((entry) => {
    ...
    if (oid !== undefined) {
      const r = RECIPES.find((r) => r.id === oid);
      if (r) return { ...entry, recipeId: r.id, recipe: r };
    }
    return entry;
  });
  ...
  const slideClass = !wasSwipe && slideDir === "in"
    ? "animate-slide-in"
    ...
    return (
      ...
      >
        <div className="flex items-center justify-between px-4 h-11">
          ...
          <div className="flex items-center gap-3">
            {jiggleMode && v.type !== "valitse-resepti" && !isBack && (
              <button onClick={() => setJiggleMode(false)} className="text-[17px] font-semibold" style={{ color: "var(--ios-blue)" }}>
                {t("done", locale)}
              </button>
            )}
        ...
        {v.type === "viikko" && !jiggleMode && (
          <div className="px-4 pb-2"><DieettiTabs /></div>
        )}
      </header>
    );
  }
  ...
  // Use snapshot views during swipe to prevent flash
  ...
  return (
    <div className="relative h-[100dvh] overflow-hidden" style={{ background: "var(--background)" }}
      ...
      {isSwiping ? (
        <>
          {displayBack && renderFullScreen(displayBack, true, {
            transform: `scale(${prevScale})`, opacity: prevOpacity, transition, zIndex: 1,
          })}
          {renderFullScreen(displayFront, false, {
            transform: `translateX(${swipe.dragX}px)`,
            boxShadow: swipe.dragX > 0 ? "-8px 0 30px rgba(0,0,0,0.3)" : "none",
            transition, zIndex: 2,
          })}
        </>
      ) : (
